...
Description:    Use this script to export/update your fitness data from Garmin Connect.
...
PARSER.add_argument(
    '--privacy', help="public, private, subscribers, groups", nargs='?')
...
OPENER = urllib.request.build_opener(urllib.request.HTTPCookieProcessor(
    COOKIE_JAR), urllib.request.HTTPSHandler(debuglevel=0))
# print(COOKIE_JAR)
...
def http_req(url, post=None, headers=None):
    ...
    if post:
        ...
        # Convert dictionary to POST parameter string.
    ...
    if response.getcode() == 204:
        ...
        raise Exception(
            'Bad return code (' + str(response.getcode()) + ') for: ' + url)
...
print('  Currently this tool supports changing your activity type to either cycling, hiking or running')
print('')
USERNAME = ''
PASSWORD = ''
while not USERNAME:
...
print('  Up to ' + str(LIMIT_ACTIVITY_LIST) +
      ' activities can be processed at one time.')
...
STARTDATE = promptDate(
    '  Start Date (e.g. "2018-09-30" or blank): ', ARGS.startdate, "  Invalid date.")
ENDDATE = promptDate(
    '  End Date (e.g. "2018-10-30" or blank): ', ARGS.enddate, "  Invalid date.")
print('')
...
LOGIN_RESPONSE = http_req(
    URL_GC_LOGIN + '#', urllib.parse.urlencode(POST_DATA), headers).decode()
...
#    print('Activity: ' + str(a))
    print('Activity distance: ' + str(a['distance']))
    print('Activity average speed: ' + str(a['averageSpeed']))

    if a['locationName'] is None:
        ACTIVITY_NAME = ''
    else:
        ACTIVITY_NAME = a['locationName'] + ' '

    if float(a['distance']) > 15000.0:
        print('Cycling')
        ACTIVITY_NAME = ACTIVITY_NAME + 'Cycling'
        ACTIVITY_TYPE_ID = 2
        ACTIVITY_TYPE = 'cycling'
    elif float(a['averageSpeed']) > 1.6667:
        print('Running')
        ACTIVITY_NAME = ACTIVITY_NAME + 'Running'
        ACTIVITY_TYPE_ID = 1
        ACTIVITY_TYPE = 'running'
    elif float(a['averageSpeed']) <= 1.6667:
        print('Walking')
        ACTIVITY_NAME = ACTIVITY_NAME + 'Walking'
        ACTIVITY_TYPE_ID = 3
    ...
    ACTIVITY_POST_DATA = {'activityTypeDTO': {'typeId': ACTIVITY_TYPE_ID,
                                              'typeKey': ACTIVITY_TYPE, 'parentTypeId': 17}, 'activityId': a['activityId'], 'activityName': ACTIVITY_NAME}
    ...
    print('About to post: ' + json.dumps(ACTIVITY_POST_DATA))
    http_req(ACTIVITY_URL, json.dumps(
        ACTIVITY_POST_DATA), ACTIVITY_POST_HEADERS)
